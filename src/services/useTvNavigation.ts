...
export function useTvNavigation({ enabled = true }: UseTvNavigationOptions = {}) {
  ...
  useEffect(() => {
    ...
    const handleTvKeyDown = (e: KeyboardEvent) => {
      ...
      if (isUp || isDown || isLeft || isRight || isTab) {
        isNavKeyDown = true;
      }
      ...
      if (activeEl.getAttribute('data-tv-nav') === 'category') {
        ...
        if (isRight && currIdx >= 0 && currIdx < categories.length - 1) {
          ...
          nextCat.focus();
          nextCat.scrollIntoView({ behavior: 'auto', block: 'nearest', inline: 'nearest' });
          return;
        }
        ...
        if (isLeft && currIdx > 0) {
          ...
          prevCat.focus();
          prevCat.scrollIntoView({ behavior: 'auto', block: 'nearest', inline: 'nearest' });
          return;
        }
        ...
        if (isHorizontalCarousel) {
          ...
          if (isLeft) {
            ...
            if (prevCard) {
              e.preventDefault();
              prevCard.focus();
              prevCard.scrollIntoView({ behavior: 'auto', block: 'nearest', inline: 'nearest' });
          ...
          if (isUp) {
            ...
            if (activeCategory) {
              ...
              activeCategory.scrollIntoView({ behavior: 'auto', block: 'center', inline: 'nearest' });
            }
            return;
          }
        }
